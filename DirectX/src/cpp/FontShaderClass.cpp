...
bool FontShaderClass::Initialize(ID3D11Device* device, HWND hwnd)
{
    ...
    error = wcscpy_s(vsFileName, 128, L"./src/shaders/fontvs.hlsl");
    if (error != 0)
    {
        return false;
    }
    error = wcscpy_s(psFileName, 128, L"./src/shaders/fontps.hlsl");
    if (error != 0)
    {
        return false;
    }
...
bool FontShaderClass::Render(ID3D11DeviceContext* deviceContext, int indexCount, XMMATRIX worldMatrix, XMMATRIX viewMatrix,
								XMMATRIX projectionMatrix, ID3D11ShaderResourceView* texture, XMFLOAT4 a_PixelColor)
{
	bool result;


	// Set the shader parameters that it will use for rendering.
	result = SetShaderParameters(deviceContext, worldMatrix, viewMatrix, projectionMatrix, texture, a_PixelColor);
	if(!result)
	{
		return false;
	}
											 ...
											 XMMATRIX projectionMatrix, ID3D11ShaderResourceView* texture, XMFLOAT4 a_PixelColor)
{
	...
	dataPtr2->pixelColor = a_PixelColor;
